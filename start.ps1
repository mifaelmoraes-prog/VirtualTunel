...
$backendProcess = Start-Process powershell `
    -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot\backend'; .\venv\Scripts\python.exe main.py" `
...
$frontendProcess = Start-Process powershell `
    -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot\frontend'; npm run dev -- --port 5173 --strictPort" `
    -WindowStyle Minimized -PassThru

# Aguarda os servidores ficarem prontos
Write-Host "  Aguardando serviços estabilizarem... (4s)" -ForegroundColor DarkGray
...
# Garante que portas sejam liberadas após o encerramento
foreach ($port in $targetPorts) {
    $lines = netstat -ano | Select-String "[:.]$port\s"
    foreach ($line in $lines) {
        $parts = ($line -replace '\s+', ' ').Trim() -split ' '
