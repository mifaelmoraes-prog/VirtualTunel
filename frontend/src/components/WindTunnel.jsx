...
const PressureSmoke = ({ slices, showPressure, windSpeedFactor = 1 }) => {
  ...
  const { positions, colors, opacities, sizes, velocities, life } = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const speed = (1.2 + Math.random() * 0.3) * windSpeedFactor;
      vel[i * 3] = -speed;
      vel[i * 3 + 1] = (Math.random() - 0.5) * 0.03;
      vel[i * 3 + 2] = (Math.random() - 0.5) * 0.03;

      opa[i] = 0;
      siz[i] = 0.15 + Math.random() * 0.2;
...
const StreamlinesWithSlices = ({ sliceCount, showStreamlines }) => {
  ...
  const baseSpeed = 0.2 * windSpeedFactor;
...
const VortexVisualization = ({ windSpeedFactor = 1 }) => {
  ...
  const { positions, colors, sizes } = useMemo(() => {
    ...
    const siz = new Float32Array(vortexCount);

    for (let i = 0; i < vortexCount; i++) {
      const isLeft = i % 2 === 0;
      pos[i * 3] = -2.5 + (Math.random() - 0.5) * 0.5;
      pos[i * 3 + 1] = isLeft ? 1.0 : 0.8;
      pos[i * 3 + 2] = isLeft ? 0.8 : -0.8;

      col[i * 3] = 0.9;
      col[i * 3 + 1] = 0.4;
      col[i * 3 + 2] = 0.2;

      siz[i] = 0.3 + Math.random() * 0.2;
    }
  ...
  useFrame(({ clock }) => {
    ...
    const t = clock.getElapsedTime() * windSpeedFactor;
    const posAttr = vorticesRef.current.geometry.attributes.position;
    const sizeAttr = vorticesRef.current.geometry.attributes.size;

    for (let i = 0; i < vortexCount; i++) {
      const isLeft = i % 2 === 0;
      const offset = Math.sin(t * 2 + i) * 0.3;

      posAttr.array[i * 3] = -2.5 - t * 0.5 + offset;
      posAttr.array[i * 3 + 1] = (isLeft ? 1.0 : 0.8) + Math.cos(t * 3 + i) * 0.1;
      posAttr.array[i * 3 + 2] = (isLeft ? 0.8 : -0.8) + Math.sin(t * 2 + i) * 0.15;

      sizeAttr.array[i] = (0.3 + Math.random() * 0.1) * (1 + Math.sin(t * 5 + i) * 0.2);
    }
  ...
  return (
    <points ref={vorticesRef}>
      <bufferGeometry>
        <bufferAttribute attach="attributes-position" count={vortexCount} array={positions} itemSize={3} />
        <bufferAttribute attach="attributes-color" count={vortexCount} array={colors} itemSize={3} />
        <bufferAttribute attach="attributes-size" count={vortexCount} array={sizes} itemSize={1} />
      </bufferGeometry>
      <shaderMaterial
        transparent
        depthWrite={false}
...
const WindTunnel = ({ sliceCount = 8, materialSettings = {}, showStreamlines = true, showPressure = true, showSmoke = true, showVortices = false, windSpeed = 20, modelYaw = 0 }) => {
  const slices = useMemo(() => {
    const result = [];
    const xMin = -2.0;
    const xMax = 2.0;
  ...
  const windSpeedFactor = windSpeed / 20;
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: '#0a0a0a' }}>
      <Canvas shadows>
        ...
        <ambientLight intensity={0.25} color="#404050" />
        <spotLight
          position={[8, 14, 6]}
          intensity={2.5 * windSpeedFactor}
          color="#fff5e6"
          angle={0.4}
          penumbra={0.3}
          castShadow
        ...
        <spotLight
          ...
          penumbra={0.7}
        />
        <pointLight position={[0, 6, 0]} intensity={0.6 * windSpeedFactor} color="#ff6633" />
        <spotLight position={[0, 8, 8]} intensity={1.0} color="#ffffff" angle={0.5} penumbra={0.6} />
        ...
        <StreamlinesWithSlices sliceCount={sliceCount} showStreamlines={showStreamlines} windSpeedFactor={windSpeedFactor} />
        <PressureSmoke slices={slices} showPressure={showSmoke} windSpeedFactor={windSpeedFactor} />
        {showVortices && <VortexVisualization windSpeedFactor={windSpeedFactor} />}
